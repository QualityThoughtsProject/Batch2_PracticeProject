...
public class CommonMethods {
	...
	public static void Explicit_Wait(WebElement element) {
		...
		WebDriverWait wait = new WebDriverWait(DriverFactory.getCurrentDriver(),30);
	...
	/*public static void clickOnElement(WebElement element, String log_commands) {
		try {
			...
			TestReport.log(LogStatus.FAIL, "Unable to click Element "+e.getMessage());
		}
	}*/


	/*static String getText(WebElement element, String log_commands) {
		try {
			String gettext = element.getText();
			highLight(element);
			TestReport.logNoScreen(LogStatus.INFO, "Text Found "+gettext);
			return gettext;
		} catch (NoSuchElementException e) {
			TestReport.log(LogStatus.FAIL, "Unable to get text "+e.getMessage());
		}
		return null;
	}*/
...
public static void highLight(By by) {
 ...
 public static void closeWindow() {
	...
	public void veriffPdfContent() throws MalformedURLException, IOException{
		BufferedInputStream pdfUrl = new BufferedInputStream(new URL(DriverFactory.getCurrentDriver().getCurrentUrl()).openStream());
